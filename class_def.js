...
var map = new Map();
map.set('switch', new('switch', 'sw', 's31', 's31'));
for (var i = 0; i < 5; i++) {

	if(i==0 || i==2) {
		map.set('t'+i, new('t'+i, 'hm', 'c'+i+'1', 's'+i+'3'));
	} else {
		map.set('t'+i, new('t'+i, 'hm', 'c'+i+'1', 'c'+i+'1'));
	}

	if( i==0 || i==2 ) {
		for (var j = 1; j < 4; j++) {
				var nxt = (j==1) ? 'switch' : 's'+i+''+(j-1);			
				map.set('s'+i+''+j, new('s'+i+''+j, 'no', nxt, nxt));
	...
	for (var j = 1; j < 6; j++) {
		if(j==5) {
			map.set('c'+i+''+j, new('c'+i+''+j, 'no', 't'+((i+1)%5), 't'+((i+1)%5)));
		} else if( (i==0 || i==3) && j==3) {
			map.set('c'+i+''+j, new('c'+i+''+j, 'dh', 'c'+i+'4', 'c'+i+'4'));
		} else {
			map.set('c'+i+''+j, new('c'+i+''+j, 'no', 'c'+i+''+(j+1), 'c'+i+''+(j+1));
		}
    }	
